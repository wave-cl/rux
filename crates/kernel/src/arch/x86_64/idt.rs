...
unsafe fn call_on_irq_stack(func: unsafe fn(u64, u64, *mut u8), vector: u64, error_code: u64, frame: *mut u8) {
    let irq_stack_top = crate::percpu::this_cpu().irq_stack_top;
    if irq_stack_top == 0 {
        ...
        32 => {
            ...
            unsafe {
                ...
                if rux_net::is_configured() {
                    ...
                    let activity = rux_net::poll(crate::arch::Arch::ticks());
                    // Only wake poll waiters when there's actual network activity
