...
pub fn sys_open(path: &[u8]) -> isize {
    unsafe { rux_vfs::fdtable::sys_open(path, crate::kstate::fs()) }
}

/// Open a file by inode with flags.
pub fn sys_open_ino(ino: rux_vfs::InodeId, flags: u32) -> isize {
...
pub fn sys_read_fd(fd: usize, buf: *mut u8, len: usize) -> isize {
    unsafe {
...
pub fn sys_write_fd(fd: usize, buf: *const u8, len: usize) -> isize {
    unsafe {
