...
///
/// This is the glue between rux-sched's CFS implementation and the actual
/// context switch assembly. The timer ISR calls `schedule_tick()`, which
/// advances vruntime and context-switches if needed.
...
pub struct KernelTask {
    ...
    /// Saved RSP for context_switch (callee-saved regs + return address on stack).
    pub saved_rsp: u64,
...
impl Scheduler {
    pub const fn new() -> Self {
        ...
        // schedule() saves kernel_main's RSP here when switching to a real task,
    ...
    pub unsafe fn create_task(
        ...
        // Initialize the kernel stack so context_switch "returns" to entry
        {
            ...
            task.saved_rsp = crate::arch::Arch::init_task_stack(stack_top, entry as u64, 0);
        }
    ...
    pub unsafe fn schedule(&mut self) {
        ...
        // Pick the next task
        // We need a "prev" entity for pick_next — use a dummy
        ...
        if new_idx != old_idx {
            ...
            use rux_arch::ContextOps;
            crate::arch::Arch::context_switch(old_rsp_ptr, new_rsp);
        }
    }
}
