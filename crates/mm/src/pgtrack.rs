...
use crate::{FrameAllocator, MemoryError, PageSize};
...
///
/// `write_pt_root` is the arch-specific function to write the page table root
/// register (e.g., CR3 on x86_64, TTBR0_EL1 on aarch64).
pub fn begin_child(alloc: &mut dyn FrameAllocator, write_pt_root: unsafe fn(u64)) {
    unsafe {
        // Switch to kernel page table before freeing child pages,
        // because the child's page table pages are about to be freed.
        if KERNEL_PT_ROOT != 0 {
            write_pt_root(KERNEL_PT_ROOT);
        }
