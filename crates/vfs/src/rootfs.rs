...
/// files at boot. All commands are symlinks to `/bin/busybox`.
///
/// This module is architecture-independent. Progress logging is done via
...
// All target "busybox" (relative symlink in same directory)
...
/// `elf_data` is the busybox ELF binary for this architecture.
/// `log` is an optional logging callback for progress messages.
pub fn populate(
        ...
        let n = match fs.write(box_ino, offset, chunk) {
            ...
            Err(_) => {
                if let Some(log_fn) = log {
                    log_fn("rootfs: write failed at page ");
                    let mut buf = [0u8; 10];
                    log_fn(format_u32(&mut buf, pages_written));
                    log_fn("\n");
                }
                break;
            }
...
/// Format a u32 as decimal into a buffer, returning a &str slice.
