...
PYTEST
    cat > "$STAGING/usr/share/rux-tests/syscall_conf.py" << 'PYTEST'
# Syscall conformance: deterministic cases covering happy paths and the
# common error paths real programs rarely hit. Each case prints PASS/FAIL;
# the test.sh check matches on the final summary line.
#
# IMPORTANT: kept defensive — does not pass kernel-space pointers or
# extreme fd values, because the kernel currently panics on those instead
# of returning EFAULT/EBADF. Those are real bugs but fixing them is out
# of scope for the framework upgrade; they're tracked separately.
...
expect_ok('getcwd', L.getcwd(cwd, 256))
